...
set_sysctl() {
    msg_info "配置内核参数..."
    local conf_d="/etc/sysctl.d/99-init-sysctl.conf"
    local conf_old="/etc/sysctl.conf"
    local cfg_content
    local line
    local key
    local escaped_key
...
EOF
    ...
    if [ ! -f "$conf_d" ]; then
        echo "$cfg_content" > "$conf_d"
    else
        while IFS= read -r line; do
            [ -z "$line" ] && continue
            if [[ "$line" =~ ^[[:space:]]*# ]]; then
                grep -Fqx "$line" "$conf_d" 2>/dev/null || echo "$line" >> "$conf_d"
                continue
            fi
            key=$(echo "$line" | awk -F= '{print $1}' | sed 's/[[:space:]]*$//')
            escaped_key=$(echo "$key" | sed 's/\./\\./g')
            sed -i -E "/^[[:space:]]*#?[[:space:]]*${escaped_key}[[:space:]]*=.*/d" "$conf_d"
            echo "$line" >> "$conf_d"
        done <<< "$cfg_content"
    fi
    if [ -f "$conf_old" ]; then
        while IFS= read -r line; do
            ...
            key=$(echo "$line" | awk -F= '{print $1}' | sed 's/[[:space:]]*$//')
            escaped_key=$(echo "$key" | sed 's/\./\\./g')
            if grep -qE "^[[:space:]]*${escaped_key}[[:space:]]*=" "$conf_old" 2>/dev/null; then
                sed -i "s|^[[:space:]]*${escaped_key}[[:space:]]*=.*$|# &  # disabled by startsys.sh|" "$conf_old"
            fi
        done <<< "$cfg_content"
    fi
    msg_info "应用 sysctl 配置..."
    /sbin/sysctl -p "$conf_d" 2>/dev/null | awk -F' = ' -v g="$GREEN" -v o="$OK" -v p="$PLAIN" '{printf "  %s%s%s %-35s = %s\n", g, o, p, $1, $2}' || true
    msg_ok "内核参数配置完成"
}
