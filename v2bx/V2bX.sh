...
config() {
    ...
    vi /opt/V2bX/config/config.json
    sleep 2
    restart
    check_status
    case $? in
...
uninstall() {
    ...
    rm /opt/V2bX/config/ -rf
    rm /opt/V2bX/ -rf

    echo ""
    echo -e "卸载成功，如果你想删除此脚本，则退出脚本后运行 ${green}rm /usr/local/bin/V2bX -f${plain} 进行删除"
    echo ""
...
update_shell() {
    wget -O /usr/local/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/terminodev/titanicsite7/main/v2bx/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
    ...
    node_config=$(cat <<EOF
{
            ...
            "CertConfig": {
                ...
                "CertFile": "/opt/V2bX/config/fullchain.cer",
                "KeyFile": "/opt/V2bX/config/cert.key",
    ...
    node_config=$(cat <<EOF
{
            ...
            "NodeID": $NodeID,
            "NodeType": "$NodeType",
            "Hysteria2ConfigPath": "/opt/V2bX/config/hy2config.yaml",
            "Timeout": 30,
            "ListenIP": "",
            "SendIP": "0.0.0.0",
            "DeviceOnlineMinTraffic": 200,
...
generate_config_file() {
    ...
    echo -e "${red}2. 生成的配置文件会保存到 /opt/V2bX/config/config.json${plain}"
    echo -e "${red}3. 原来的配置文件会保存到 /opt/V2bX/config/config.json.bak${plain}"
        ...
        cores_config+="
    {
        ...
        \"OriginalPath\": \"/opt/V2bX/config/sing_origin.json\"
    },"
    fi
    ...
    # 创建 config.json 文件
    cat <<EOF > /opt/V2bX/config/config.json
{
    "Log": {
        "Level": "error",
        "Output": ""
...
EOF
    ...
    cat <<EOF > /opt/V2bX/config/custom_outbound.json
    [
        {
            "tag": "IPv4_out",
            "protocol": "freedom",
...
EOF
    ...
    # 创建 route.json 文件
    cat <<EOF > /opt/V2bX/config/route.json
    {
        "domainStrategy": "AsIs",
        "rules": [
            {
...
EOF
    ...
    cat <<EOF > /opt/V2bX/config/sing_origin.json
{
  "dns": {
    "servers": [
      {
...
EOF
    ...
    cat <<EOF > /opt/V2bX/config/hy2config.yaml
quic:
