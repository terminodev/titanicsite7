...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/terminodev/titanicsite7/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 V2bX 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 V2bX 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 V2bX 最新版本：${last_version}，开始安装"
        wget --no-check-certificate -N --progress=bar -O /opt/V2bX/V2bX-linux.zip https://github.com/terminodev/titanicsite7/releases/download/${last_version}/V2bX-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/terminodev/titanicsite7/releases/download/${last_version}/V2bX-linux-${arch}.zip"
        echo -e "开始安装 V2bX $1"
        wget --no-check-certificate -N --progress=bar -O /opt/V2bX/V2bX-linux.zip ${url}
        if [[ $? -ne 0 ]]; then
    ...
    rm V2bX-linux.zip -f
    chmod +x V2bX
    mkdir /opt/V2bX/config/ -p
    cp geoip.dat /opt/V2bX/config/
    cp geosite.dat /opt/V2bX/config/
    if [[ x"${release}" == x"alpine" ]]; then
...
command="/opt/V2bX/V2bX"
command_args="server -c /opt/V2bX/config/config.json"
command_user="root"
...
LimitCORE=infinity
LimitNOFILE=999999
WorkingDirectory=/opt/V2bX/
ExecStart=/opt/V2bX/V2bX server -c /opt/V2bX/config/config.json
Restart=always
RestartSec=10
...
EOF
    ...
    if [[ ! -f /opt/V2bX/config/config.json ]]; then
        cp config.json /opt/V2bX/config/
        echo -e ""
    ...
    if [[ ! -f /opt/V2bX/config/dns.json ]]; then
        cp dns.json /opt/V2bX/config/
    fi
    if [[ ! -f /opt/V2bX/config/route.json ]]; then
        cp route.json /opt/V2bX/config/
    fi
    if [[ ! -f /opt/V2bX/config/custom_outbound.json ]]; then
        cp custom_outbound.json /opt/V2bX/config/
    fi
    if [[ ! -f /opt/V2bX/config/custom_inbound.json ]]; then
        cp custom_inbound.json /opt/V2bX/config/
    fi
    curl -o /usr/local/bin/V2bX -Ls https://raw.githubusercontent.com/terminodev/titanicsite7/main/v2bx/V2bX.sh
    chmod +x /usr/local/bin/V2bX
    if [ ! -L /usr/local/bin/v2bx ]; then
        ln -s /usr/local/bin/V2bX /usr/local/bin/v2bx
        chmod +x /usr/local/bin/v2bx
    fi
    cd $cur_dir
    rm -f install.sh
    ...
    if [[ $first_install == true ]]; then
        ...
        if [[ $if_generate == [Yy] ]]; then
            curl -o ./initconfig.sh -Ls https://raw.githubusercontent.com/terminodev/titanicsite7/main/v2bx/initconfig.sh
