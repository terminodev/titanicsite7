...
install_XrayR() {
    ...
    #wget -N --no-check-certificate -O "$config_file" https://raw.githubusercontent.com/terminodev/titanicsite7/main/xrayr/config.yml.example
    #wget -N --no-check-certificate -O /opt/xrayr/dns_${xrayrname}.json https://raw.githubusercontent.com/terminodev/titanicsite7/main/xrayr/dns.json
    #if [[ $? -ne 0 ]]; then
...
EOF
    cat > /opt/xrayr/dns_${xrayrname}.json <<'EOF'
{
  "servers": [
    "1.1.1.1",
    "8.8.8.8",
...
EOF
    ...
    docker pull ghcr.io/terminodev/xrayr-backup:latest
    docker run --restart=always --log-opt max-size=5m --log-opt max-file=3 \
        ...
        ${cert_mount} \
        --network=host ghcr.io/terminodev/xrayr-backup:latest
