...
install_caddy() {
    ...
    if [ "$build" = "1" ]; then
        ...
        if [ "${SYSTEM_ARCH}" = "amd64" ] || [ "${SYSTEM_ARCH}" = "arm64" ]; then
            caddy_url=https://raw.githubusercontent.com/terminodev/titanicsite7/main/caddy/caddy_2.11.4_with_tn_linux_${SYSTEM_ARCH}.tgz
        else
...
if [ "$type" = "trojan" ]; then
    ...
    if [ "$ipv6" = "1" ]; then
		if [[ $address_ipv6 =~ ^$ipv6_regex$ ]]; then
        	trojan_host="[${address_ipv6}]"
		else
			trojan_host="[::/0]"
		fi
    else
        trojan_host="$address_ipv4"
    fi
    if [ "$ssl" = "0" ]; then
        echo -e "Sni              : ${yellow}${domain}${plain}"
        cert_fingerprint=$(openssl s_client -connect 127.0.0.1:${port} -servername ${domain} -showcerts </dev/null 2>/dev/null | sed -n '/BEGIN CERTIFICATE/,/END CERTIFICATE/p' | openssl x509 -noout -fingerprint -sha256 | cut -d= -f2 | tr -d ':' | tr 'A-Z' 'a-z')
        echo -e "证书指纹(SHA-256) : ${yellow}${cert_fingerprint}${plain}"
        echo -e "分享链接         : ${yellow}trojan://${password}@${trojan_host}:${port}?security=tls&sni=${domain}&fp=chrome&alpn=h2%2Chttp%2F1.1&insecure=1&allowInsecure=1&pcs=${cert_fingerprint}&type=tcp&headerType=none#trojan$(date +%Y%m%d%H%M)${plain}"
        echo -e "${yellow}───────────────────────────────────────────${plain}"
        echo -e "${yellow}⚠  提示 (Tip)    : 请开启跳过证书验证${plain}"
        echo -e "${yellow}                  (Enable Skip Cert Verify)${plain}"
    else
        echo -e "分享链接         : ${yellow}trojan://${password}@${trojan_host}:${port}?security=tls&sni=${domain}&fp=chrome&alpn=h2%2Chttp%2F1.1&insecure=0&allowInsecure=0&type=tcp&headerType=none#trojan$(date +%Y%m%d%H%M)${plain}"
    fi
	echo -e "${green}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${plain}"
